...
namespace SABI
{
    public static class ButtonDrawer
    {
        ...
        {
            // new Rectangle().FixedSize(25, 25).SetParent(root);

            if (attribute == null)
                return;

            Label label = new Label(attribute.customText ?? method.Name).OnHover(
                element => { },
                element => { }
            );
            ...
            Div parametersAndButton = new Div(new FColumn(parametersFoldout, button))
                .OnHover(
                    element =>
                    {
                        element
                            .BorderRadius(attribute.attributeStyle.borderRadius ?? 10)
                            .BorderWidth(1)
                            .BorderColor(attribute.attributeStyle.borderColor ?? Color.yellow);
                    },
                    element =>
                    {
                        element
                            .BorderRadius(attribute.hover_attributeStyle.borderRadius ?? 10)
                            .BorderWidth(1)
                            .BorderColor(attribute.hover_attributeStyle.borderColor ?? Color.white);
                    }
                )
                .Padding(5)
            ...
            {
                if (
                    (attribute.buttonUsage == ButtonUsage.EveryTime)
                    || (attribute.buttonUsage == ButtonUsage.RunTime && Application.isPlaying)
                    || (attribute.buttonUsage == ButtonUsage.EditorTime && !Application.isPlaying)
                )
                {
                    ...
                    Debug.Log($"Methord can only be used at {attribute.buttonUsage.ToString()}");
            });
